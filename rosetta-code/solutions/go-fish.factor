! Write a program to let the user play Go Fish against a computer
! opponent. Use the following rules:
! 
! -   Each player is dealt nine cards to start with.
! -   On their turn, a player asks their opponent for a given rank (such
!     as threes or kings). A player must already have at least one card of
!     a given rank to ask for more.
!     -   If the opponent has any cards of the named rank, they must hand
!         over all such cards, and the requester can ask again.
!     -   If the opponent has no cards of the named rank, the requester
!         draws a card and ends their turn.
