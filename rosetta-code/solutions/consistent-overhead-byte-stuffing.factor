! The Consistent Overhead Byte Stuffing (COBS) algorithm is used to
...
! (we'll call this a milestone). If 0x00 does not appear within the next
! 254 bytes after a milestone the algorithm must emit an extra milestone
! at the 255th byte. Of the encoded output, the first byte is the first
