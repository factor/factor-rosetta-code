...
! 
! No untouchable number is one more than a prime number, since if p is
! prime, then the sum of the proper divisors of p² isp+1.
! 
! No untouchable number is three more than an odd prime number, since if p
! is an odd prime, then the sum of the proper divisors of 2p isp+3.
! 
! The number 5 is believed to be the only odd untouchable number, but this
! has not been proven: it would follow from a slightly stronger version of
! the Goldbach'sconjecture, since the sum of the proper divisors of pq
! (with p, q being distinct primes) is 1+p+q.
! 
