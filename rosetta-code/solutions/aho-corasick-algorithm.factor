! The Aho–Corasick algorithm is an efficient string-searching algorithm
! designed to locate all occurrences of a finite set of pattern strings (a
! dictionary) within an input text. Developed by Alfred V. Aho and
! Margaret J. Corasick in 1975, it is widely used in applications
! requiring multiple pattern matching, such as text processing,
! bioinformatics, and network security.
! 
! Problem Description
! 
! Given a text string T of length n and a set of k pattern strings P =
! {P₁, P₂, ..., Pₖ} with a total length m (sum of lengths of all
! patterns), the Aho–Corasick algorithm finds all occurrences of any
! pattern Pᵢ in T. Specifically, it reports the starting positions in T
! where each pattern appears as a substring. The algorithm is particularly
! efficient when searching for multiple patterns simultaneously, as it
! avoids scanning the text repeatedly for each pattern.
! 
