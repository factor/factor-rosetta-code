!     
! 
!         ''This task is about derived types; for implementation
!         inheritance, see Polymorphism.
! 
! Inheritance is an operation of type algebra that creates a new type from
! one or several parent types.
! 
...
! parent types.
! 
! When all (in certain sense) properties of the parents are preserved by
! the derived type, it is said to be a Liskov subtype.
! 
! When properties are preserved then the derived type is substitutable for
! its parents in all contexts. Usually full substitutability is achievable
...
! 
! Parent types are sometimes called supertype, the derived ones are
! subtype. This relation is transitive and reflexive.
! 
! Types bound by the relation form a wp:Directed_acyclic_graph directed
! acyclic graph (ignoring reflexivity).
! 
...
!         None of the classes need to have any functions, the only thing
!         they need to do is inherit from the specified superclasses
! 
