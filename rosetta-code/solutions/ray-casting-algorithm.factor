...
!    return outside
! 
! Where the function ray_intersects_segment return true if the horizontal
! ray starting from the point P intersects the side (segment), false
! otherwise.
! 
! An intuitive explanation of why it works is that every time we cross a
! border, we change "country" (inside-outside, or outside-inside), but the
! last "country" we land on is surely outside (since the inside of the
! polygon is finite, while the ray continues towards infinity). So, if we
! crossed an odd number of borders we were surely inside, otherwise we
! were outside; we can follow the ray backward to see it better: starting
! from outside, only an odd number of crossing can give an inside:
...
! fact that rays starting from points in the hatched area (like P₁ and P₂)
! surely do not intersect the segment AB. We also can easily see that rays
! starting from points in the greenish area surely intersect the segment
! AB (like point P₃).
! 
! So the problematic points are those inside the white area (the box
! delimited by the points A and B), like P₄.
! 
! [] []
! 
...
! the images, the ray starting from P_(A) does not intersect the segment,
! while the ray starting from P_(B) in the second picture, intersects the
! segment).
! 
! Points on the boundary or "on" a vertex are someway special and through
! this approach we do not obtain coherent results. They could be treated
! apart, but it is not necessary to do so.
! 
