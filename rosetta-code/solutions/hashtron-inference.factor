...
! Description
! 
! This task involves implementing a Hashtron classifier inference
! function. Hashtron classifier program can be machine learned to
! calculate arbitrary finite computable functions or sets (Given enough
! CPU time and RAM). The Hashtron classifier takes a command and a number
! of bits to infer, using a provided program configuration. The inference
! function for the test will generate a pseudo-random n-bits output. The
! inference function for the square root demo will generate a square root
! of a command in range: 0 <= command < 256.
! 
! Examples
! 
! Test demo
...
!     [[0,2]]
! 
! The inference function should process the input and generate the
! following 64-bit output:
! 
...
! 
! The inference function should process the input and generate the square
! root of the input byte.
