...
! G9. The Lord of the Night may or may not be included in a Mayan date, if
! it is, it is typically just the appropriate G(x) at the end.
! 
! The Long Count
! 
! Mayans had a separate independent way of measuring time that did not run
! in cycles. (At least, not on normal human scales.) Instead, much like
! our yearly calendar, each day just gets a little further from the
! starting point. For the ancient Maya, the starting point was the
! ‘creation date’ of the current world. This date corresponds to our date
! of August 11, 3114 B.C. Dates are calculated by measuring how many days
! have transpired since this starting date; This is called “The Long
! Count.” Rather than just an absolute count of days, the long count is
! broken up into unit blocks, much like our calendar has months, years,
...
! 
! For the most part, the Maya only used the blocks of time up to the
! bak’tun. One bak’tun is around 394 years, much more than a human life
! span, so that was all they usually needed to describe dates in this era,
! or this world. It is worth noting, the two calendars working together
! allow much more accurate and reliable notation for dates than is
! available in many other calendar systems; mostly due to the pragmatic
! choice to make the calendar simply track days, rather than trying to
! make it align with seasons and/or try to conflate it with the notion of
! time.
! 
! Mayan Date correlations
! 
! There is some controversy over finding a correlation point between the
! Gregorian and Mayan calendars. The Gregorian calendar is full of jumps
! and skips to keep the calendar aligned with the seasons so is much more
! difficult to work with. The most commonly used correlation factor is The
! GMT: 584283. Julian 584283 is a day count corresponding Mon, Aug 11,
! 3114 BCE in the Gregorian calendar, and the final day in the last Mayan
! long count cycle: 13.0.0.0.0 which is referred to as "the day of
! creation" in the Mayan calendar. There is nothing in known Mayan writing
! or history that suggests that a long count "cycle" resets every 13
! bak’tun. Judging by their other practices, it would make much more sense
! for it to reset at 20, if at all.
! 
! The reason there was much interest at all, outside historical scholars,
! in the Mayan calendar is that the long count recently (relatively
! speaking) rolled over to 13.0.0.0.0 (the same as the historic day of
! creation Long Count date) on Fri, Dec 21, 2012 (using the most common
! GMT correlation factor), prompting conspiracy theorists to predict a
! cataclysmic "end-of-the-world" scenario.
! 
...
! The Task:
! 
! Write a reusable routine that takes a Gregorian date and returns the
! equivalent date in Mayan in the Calendar Round and the Long Count. At a
! minimum, use the GMT correlation. If desired, support other
! correlations.
! 
! Using the GMT correlation, the following Gregorian and Mayan dates are
! equivalent:
! 
...
! 
! Support looking up dates for at least 50 years before and after the
! Mayan Long Count 13 bak’tun rollover: Dec 21, 2012. (Will require taking
! into account Gregorian leap days.)
! 
! Show the output here, on this page, for at least the following dates:
! 
! (Note that these are in ISO-8601 format: YYYY-MM-DD. There is no
! requirement to use ISO-8601 format in your program, but if you differ,
! make a note of the expected format.)
! 
!    2004-06-19
!    2012-12-18
!    2012-12-21
