! Wordiff is an original game in which contestants take turns spelling new
! dictionary words of three or more characters that only differ from the
! last by a change in one letter.
! 
...
! -   Choosing an initial random three or four letter word from the
!     dictionary.
...
! -   An internal timer accumulates how long each user takes to respond in
!     their turns.
! -   Play is halted if the maximum playing time is exceeded on a players
!     input.
!     -   That last player must have entered a wordiff or loses.
!     -   If the game is timed-out, the loser is the person who took the
!         longest `average` time to answer in their rounds.
