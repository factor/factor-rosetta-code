...
! mechanism with the encapsulated task in order to prevent object's state
! corruption.
! 
...
! output. The input can be set using the method Input. The input is a
! function of time. The output can be queried using the method Output. The
! object integrates its input over the time and the result becomes the
! object's output. So if the input is K(t) and the output is S, the object
! state S is changed to S + (K(t₁) + K(t₀)) * (t₁ - t₀) / 2, i.e. it
! integrates K using the trapeze method. Initially K is constant 0 and S
! is 0.
! 
! In order to test the object:
! 
! 1.  set its input to sin (2π f t), where the frequency f=0.5Hz. The
!     phase is irrelevant.
! 2.  wait 2s
