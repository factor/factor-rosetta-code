! Experimental Verification of the NKTg Law in Earth Orbit Based on NASA’s 2025 Earth Dataset
! 
! You are encouraged to solve this task according to the task description,
! using any language you may know.
! 
! 1. Summary
! 
! The NKTg Law describes the variation of an object’s inertia using the
! function:
! 
! NKTg = f(x, v, m)
! 
! Where:
! 
! x: position v: velocity m: mass
! 
! The two central quantities of the law are:
! 
! NKTg₁ = x × p (position–momentum interaction)
! 
! NKTg₂ = (dm/dt) × p (mass variation–momentum interaction)
! 
! Where p = m × v, and dm/dt is the rate of mass loss over time.
! 
! This study publishes the experimental verification of the NKTg Law using
! Earth orbital data obtained from the NASA JPL Horizons system and
! simulates the planetary motion parameters for 2025 assuming small
! atmospheric mass loss.
! 
! 2. Research Objectives
! 
! Verify the predictive ability of the NKTg Law on planetary motion.
! 
! Analyze Earth’s orbital position, velocity, and mass variation using the
! NKTg model.
! 
! Compare simulation results with NASA’s observed data.
! 
! 3. Data
! 
! Table 1 - Actual orbital and mass data of Earth (Earth) in 2024 from NASA JPL Horizons System
! 
!   Time         x (m)          v (m/s)      m (kg)      p = m·v          NKTg₁ = x·p (NKTm)
!   ------------ -------------- ------------ ----------- ---------------- ---------------------
!   1/1/2024     1.47103×10¹¹   3.0286×10⁴   5.97×10²⁴   1.8080742×10²⁹   2.659731390426×10⁴⁰
!   4/1/2024     1.49526×10¹¹   2.9785×10⁴   5.97×10²⁴   1.7781645×10²⁹   2.65881825027×10⁴⁰
!   7/1/2024     1.52094×10¹¹   2.9292×10⁴   5.97×10²⁴   1.7487324×10²⁹   2.659717056456×10⁴⁰
!   10/1/2024    1.49599×10¹¹   2.9782×10⁴   5.97×10²⁴   1.7779854×10²⁹   2.659848378546×10⁴⁰
!   12/31/2024   1.47101×10¹¹   3.0287×10⁴   5.97×10²⁴   1.8081339×10²⁹   2.659783048239×10⁴⁰
! 
! Mass variation sources
! 
!   Component         Source                Mass (kg/year)
!   ----------------- --------------------- ----------------
!   Hydrogen escape   GOLD/ICON             −9.5×10⁷ kg
!   Helium escape     Atmospheric studies   −1.6×10⁶ kg
!   Cosmic dust       CNEOS (NASA JPL)      +4.0×10⁷ kg
!   Nuclear decay     Geophysical models    −1.6×10⁵ kg
! 
! Net Loss ≈ −5.7×10⁷ kg/year dm/dt ≈ −1.8 kg/s
! 
! Table 2 - Simulated orbital and mass data of Earth (Earth) in 2025 according to the NKTg law
! 
!   Time         x (m)           v (m/s)      m (kg)                  p = m·v (kg·m/s)        NKTg₁ = x·p (NKTm)    NKTg₂ = dm/dt·p (NKTm)   dm/dt (kg/s)
!   ------------ --------------- ------------ ----------------------- ----------------------- --------------------- ------------------------ --------------
!   1/1/2025     1.471012×10¹¹   3.0276×10⁴   5.97219×10²⁴            1.8081314416463×10²⁹    2.659783048239×10⁴⁰   −3.25463659496333×10²⁹   −1.8
!   4/1/2025     1.494953×10¹¹   2.9791×10⁴   5.97218999999998×10²⁴   1.77917502974274×10²⁹   2.659783048239×10⁴⁰   −3.20251505353693×10²⁹   −1.8
!   7/1/2025     1.520965×10¹¹   2.9282×10⁴   5.97218999999997×10²⁴   1.74874704430345×10²⁹   2.659783048239×10⁴⁰   −3.14774467974621×10²⁹   −1.8
!   10/1/2025    1.496328×10¹¹   2.9764×10⁴   5.97218999999995×10²⁴   1.7775401170325×10²⁹    2.659783048239×10⁴⁰   −3.19957221065849×10²⁹   −1.8
!   12/31/2025   1.471025×10¹¹   3.0276×10⁴   5.97218999999994×10²⁴   1.80811546251015×10²⁹   2.659783048239×10⁴⁰   −3.25460783251828×10²⁹   −1.8
! 
! Table 3 - Actual orbital and mass data of Earth (Earth) in 2025 from NASA JPL Horizons System
! 
!   Time         x (m)           v (m/s)      m (kg)
!   ------------ --------------- ------------ -------------
!   1/1/2025     1.471012×10¹¹   3.0287×10⁴   5.9722×10²⁴
!   4/1/2025     1.494953×10¹¹   2.9791×10⁴   5.9722×10²⁴
!   7/1/2025     1.520965×10¹¹   2.9291×10⁴   5.9722×10²⁴
!   10/1/2025    1.496328×10¹¹   2.9778×10⁴   5.9722×10²⁴
!   12/31/2025   1.471025×10¹¹   3.0286×10⁴   5.9722×10²⁴
! 
! Table 4 – Comparison of NKTg Simulation vs NASA in 2025
! 
!   Time         v – NKTg     v – NASA     Sai lệch (%)
!   ------------ ------------ ------------ --------------
!   1/1/2025     3.0276×10⁴   3.0287×10⁴   −0.04%
!   4/1/2025     2.9791×10⁴   2.9791×10⁴   0.00%
!   7/1/2025     2.9282×10⁴   2.9291×10⁴   −0.03%
!   10/1/2025    2.9764×10⁴   2.9778×10⁴   −0.05%
!   12/31/2025   3.0276×10⁴   3.0286×10⁴   −0.03%
! 
! 4. Conclusion
! 
! Orbital stability: The quantity NKTg₁ of Earth maintains a target
! constant, helping the planet self-adjust its velocity when the distance
! to the Sun changes.
! 
! NKTg₂ effect: With a small atmospheric mass loss rate, the value of
! NKTg₂ becomes negative, acting as a resistance mechanism that stabilizes
! the system.
! 
! Experimental Verification of the NKTg Law in Earth Orbit Based on NASA’s 2025 Earth Dataset
! 
! Given Earth orbital data from 2024:
! 
! x (position, m) v (velocity, m/s) m (mass, kg)
! 
! dm/dt = −1.8 kg/s
! 
! Compute for each date:
! 
! 1. Linear momentum
! 
!      p = m·v
! 
! 2. NKTg₁
! 
!      NKTg₁ = x·p
! 
! 3. NKTg₂
! 
!      NKTg₂ = (dm/dt)·p
! 
! 4. Compare simulated velocity with NASA observed velocity.
! 
! 5. Compute relative error (%).
! 
! Programs should output calculated values and relative error for each
! 2025 date.
! 
! See also
! 
! -   NKTgLaw – Main task overview


