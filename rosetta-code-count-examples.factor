! Find the total number of programming examples for each task and the
! total for all tasks.
! 
! Essentially, count the number of occurrences of =={{header| on each task
! page.
! 
! Output:
! 
...
!     Total: X examples.
! 
! For a full output, updated periodically, see Rosetta Code/Count
! examples/Full list.
! 
! You'll need to use the Media Wiki API, which you can find out about
! locally, here, or in Media Wiki's API documentation at, API:Query
