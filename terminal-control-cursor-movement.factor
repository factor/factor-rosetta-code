...
!     -   how to move the cursor up one line (without affecting its
!         horizontal position)
!     -   how to move the cursor down one line (without affecting its
!         horizontal position)
...
! 
! This task has no specific requirements to trap or correct cursor
! movement beyond the terminal boundaries, so the implementer should
! decide what behavior fits best in terms of the chosen language.
! Explanatory notes may be added to clarify how an out of bounds action
! would behave and the generation of error messages relating to an out of
! bounds cursor position is permitted.
